...
class Evaluator:
    ...
    def __call__(self, formula, sess, document, context_formula=None, placeholder_id=1):
        ...
        if context_formula:
            context_formula.replace_placeholder(
                formula, placeholder_id, conservative=True)
        else:
    ...
    def train_model(self, obj, sess, no_obj=False):
        ...
        data, labels = zip(
            *[(np.array(self.h_all(pickle.loads(f.formula))), float(f.chosen)) for f in saved_formulae])
    ...
    def h_3tree(self, formula, obj, length=8):
        ...
        for elementary_formula in elementary_formulae:
            ...
            db_elementary_formula = model.objects.get(
                formula=pickled_elementary_formula)
